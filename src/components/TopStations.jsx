// // src/components/TopStations.jsx
...
import React from 'react';
import { motion } from 'framer-motion';
import { TOP_STATIONS } from "@/config";

export default function TopStations({ onPlay }) {
  return (
    <div className="mb-4 sm:mb-6">
      <h3 className="text-base sm:text-lg text-[#F97316] font-medium">Emisoras principales</h3>
      <ul className="text-[#F1F5F9]">
        {TOP_STATIONS.map((station) => (
          <motion.li
            key={station.stationuuid}
            whileHover={{
              scale: 1.05,
            ...
            className="py-1 sm:py-2 flex items-center gap-2 hover:text-[#F97316] cursor-pointer rounded"
            onClick={() => onPlay(station)}
            role="button"
          ...
          >
            ...
            />
            <div>
              <p className="font-semibold text-sm sm:text-base">{station.name}</p>
